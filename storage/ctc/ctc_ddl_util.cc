...
void *ctc_ddl_alloc_mem(char **mem_start, char *mem_end,
                        size_t malloc_size) {
  assert(mem_start != NULL && *mem_start != NULL && mem_end != NULL);
  if (*mem_start + malloc_size >= mem_end) {
    ctc_log_error("alloc ddl mem failed,size:%u", (uint32_t)malloc_size);
    return NULL;
  }
  void *ptr = *mem_start;
...
int check_ctc_identifier_name(const char *in_name) {
  ...
  if (strlen(in_name) > CTC_IDENTIFIER_MAX_LEN) {
...
ctc_alter_column_type ctc_ddl_get_alter_column_type_from_mysql_type(
  ...
  switch (alter_column_type) {// SET_DEFAULT, DROP_DEFAULT, RENAME_COLUMN
    case Alter_column::Type::SET_DEFAULT:
      return CTC_ALTER_COLUMN_SET_DEFAULT;
    case Alter_column::Type::DROP_DEFAULT:
      return CTC_ALTER_COLUMN_DROP_DEFAULT;
    case Alter_column::Type::RENAME_COLUMN:
      return CTC_ALTER_COLUMN_RENAME_COLUMN;
    case Alter_column::Type::SET_COLUMN_VISIBLE:
      return CTC_ALTER_COLUMN_SET_COLUMN_VISIBLE;
    case Alter_column::Type::SET_COLUMN_INVISIBLE:
      return CTC_ALTER_COLUMN_SET_COLUMN_INVISIBLE;
    default:
      return CTC_ALTER_COLUMN_UNKNOW;
  }
}

bool get_ctc_key_type(const KEY *key_info, int32_t *ret_type) {
  *ret_type = CTC_KEYTYPE_UNKNOW;
  if (key_info->flags & HA_SPATIAL) {
    *ret_type = CTC_KEYTYPE_SPATIAL;
  } else if (key_info->flags & HA_NOSAME) {
    if (!my_strcasecmp(system_charset_info, key_info->name, primary_key_name)) {
      *ret_type = CTC_KEYTYPE_PRIMARY;
    } else {
      *ret_type = CTC_KEYTYPE_UNIQUE;
    }
  } else if (key_info->flags & HA_FULLTEXT) {
    *ret_type = CTC_KEYTYPE_FULLTEXT;
  } else if (key_info->flags & HA_GENERATED_KEY) {
    *ret_type = CTC_KEYTYPE_FOREIGN;
  } else {
    *ret_type = CTC_KEYTYPE_MULTIPLE;
  }
  switch (*ret_type) {
    case CTC_KEYTYPE_SPATIAL:
      *ret_type = CTC_KEYTYPE_UNKNOW;
      CTC_ENGINE_ERROR("unsupport index type:CTC_KEYTYPE_SPATIAL");
      return false;
    case CTC_KEYTYPE_FULLTEXT:
      *ret_type = CTC_KEYTYPE_UNKNOW;
      CTC_ENGINE_ERROR("unsupport index type:CTC_KEYTYPE_FULLTEXT");
      return false;
    default:
      break;
  }
  ...
  *ret_algorithm = (int32_t)CTC_HA_KEY_ALG_BTREE;
  switch (algorithm) {
    case HA_KEY_ALG_RTREE:
      ctc_log_error("unsupport index hash_type:HA_KEY_ALG_RTREE");
      return true;
    case HA_KEY_ALG_SE_SPECIFIC:
      CTC_ENGINE_ERROR("unsupport index hash_type:HA_KEY_ALG_SE_SPECIFIC");
      return false;
    case HA_KEY_ALG_FULLTEXT:
      CTC_ENGINE_ERROR("unsupport index hash_type:HA_KEY_ALG_FULLTEXT");
      return false;
    case CTC_HA_KEY_ALG_HASH:
      // ctc_log_error("unsupport index hash_type:CTC_HA_KEY_ALG_HASH");
      return true;
    default:
      break;
  }
  ...
  static map<const ha_key_alg, const ctc_ha_key_alg>
      g_ctc_key_algorithm_map = {
          ...
          {HA_KEY_ALG_BTREE, CTC_HA_KEY_ALG_BTREE},
          {HA_KEY_ALG_RTREE, CTC_HA_KEY_ALG_RTREE},
          {HA_KEY_ALG_HASH, CTC_HA_KEY_ALG_HASH},
          {HA_KEY_ALG_FULLTEXT, CTC_HA_KEY_ALG_FULLTEXT}};
  auto it = g_ctc_key_algorithm_map.find(algorithm);
  if (it != g_ctc_key_algorithm_map.end()) {
...
bool ctc_ddl_get_data_type_from_mysql_type(Field *field,
  ...
  ctc_log_error("unsupport mysql datatype:%d", (int32_t)mysql_type);
  *ret_type = CTC_DDL_TYPE_UNKNOW;
  return false;
}
...
bool set_column_datatype(size_t set_num, TcDb__CtcDDLColumnDef *column) {
  if (set_num < 9) {
    column->datatype->datatype = CTC_DDL_TYPE_TINY;
    return true;
...
ctc_ddl_fk_rule ctc_ddl_get_foreign_key_rule(fk_option rule) {
  switch (rule) {
    ...
    case FK_OPTION_DEFAULT:
      return CTC_DDL_FK_RULE_RESTRICT;
    case FK_OPTION_CASCADE:
      return CTC_DDL_FK_RULE_CASCADE;
    case FK_OPTION_SET_NULL:
      return CTC_DDL_FK_RULE_SET_NULL;
    default:
      break;
  }
  ctc_log_error("unknown foreign key option %d", (int)rule);
  assert(0);
  return CTC_DDL_FK_RULE_UNKNOW;
}
...
const dd::Index *ctc_ddl_get_index_by_name(const dd::Table *tab_obj,
...
const dd::Column *ctc_ddl_get_column_by_name(const dd::Table *table_def,
                                             const char *col_name) {
  ...
  *ret_type = CTC_KEYTYPE_UNKNOW;
  switch (type) {
    ...
    case dd::Index::enum_index_type::IT_FULLTEXT:
      CTC_ENGINE_ERROR("unsupport index type:IT_FULLTEXT");
      return false;
    case dd::Index::enum_index_type::IT_SPATIAL:
      CTC_ENGINE_ERROR("unsupport index type:IT_SPATIAL");
      return false;
    default:
      break;
  }
...
bool ctc_ddl_get_create_key_algorithm(dd::Index::enum_index_algorithm algorithm,
                                      int32_t *ret_algorithm) {
  *ret_algorithm = (int32_t)CTC_HA_KEY_ALG_BTREE;
  switch (algorithm) {
    case dd::Index::enum_index_algorithm::IA_RTREE:
      // ctc_log_error("unsupport index hash_type:IA_RTREE,use CTC_HA_KEY_ALG_BTREE");
      return true;
    case dd::Index::enum_index_algorithm::IA_SE_SPECIFIC:
      CTC_ENGINE_ERROR("unsupport index hash_type:IA_SE_SPECIFIC");
      return false;
    case dd::Index::enum_index_algorithm::IA_FULLTEXT:
      CTC_ENGINE_ERROR("unsupport index hash_type:IA_FULLTEXT");
      return false;
    case dd::Index::enum_index_algorithm::IA_HASH:
      // ctc_log_error("unsupport index hash_type:IA_HASH USE CTC_HA_KEY_ALG_BTREE");
      return true;
    default:
      break;
  }

  static map<const dd::Index::enum_index_algorithm, const ctc_ha_key_alg>
      g_ctc_create_key_algorithm_map = {
          {dd::Index::enum_index_algorithm::IA_SE_SPECIFIC, CTC_HA_KEY_ALG_SE_SPECIFIC},
          {dd::Index::enum_index_algorithm::IA_BTREE, CTC_HA_KEY_ALG_BTREE},
          {dd::Index::enum_index_algorithm::IA_RTREE, CTC_HA_KEY_ALG_RTREE},
          {dd::Index::enum_index_algorithm::IA_HASH, CTC_HA_KEY_ALG_HASH},
          {dd::Index::enum_index_algorithm::IA_FULLTEXT, CTC_HA_KEY_ALG_FULLTEXT}};
  auto it = g_ctc_create_key_algorithm_map.find(algorithm);
  if (it != g_ctc_create_key_algorithm_map.end()) {
    *ret_algorithm = (int32_t)it->second;
    return true;
  }
  ...
  *ctc_part_type = CTC_PART_TYPE_INVALID;
  switch (mysql_part_type) {
    ...
    case dd::Table::PT_LIST_COLUMNS:
      *ctc_part_type = CTC_PART_TYPE_LIST;
      break;
    ...
    case dd::Table::PT_LINEAR_KEY_55:
      *ctc_part_type = CTC_PART_TYPE_HASH;
      break;
    default :
      ...
      ctc_log_system("unsupported partition type : %d.", mysql_part_type);
      return 1;
  }
  return 0;
}
  ...
  *ctc_part_type = CTC_PART_TYPE_INVALID;
  switch (mysql_subpart_type) {
    ...
    default :
      ...
      ctc_log_system("unsupported partition type : %d.", mysql_subpart_type);
      return 1;
  }
  return 0;
}
