...
int convert_ctc_error_code_to_mysql_impl(ct_errno_t error, const char* funcName, const int line) {
  ...
  if (iter != err_code_lookup_map.end()) {
   ...
   ctc_log_system("func %s(line%d) returned with unknown err, cantian ret %d", funcName, line, (int)error);
  }
  ctc_log_note("func %s(line%d) returned with errCode %d, cantian ret %d", funcName, line, ret, (int)error);
  return ret;
}
