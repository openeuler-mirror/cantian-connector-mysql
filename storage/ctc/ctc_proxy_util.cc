...
#define CTC_CONN_MAX_RETRY_TIMES 10
#define CTC_PASSWORD_BUFFER_SIZE (uint32)512
...
static int ctc_get_agent_info(char *password, uint password_size) {
  char *user_password = getenv("MYSQL_AGENT_PASSWORD");
  if (user_password == NULL) {
    return -1;
  }
  if (is_dangerous_pwd(user_password)) {
    ctc_log_error("checking pwd failed");
    return -1;
  }
  ...
  if (fp == NULL) {
    ctc_log_error("encrypting cmd failed");
    return -1;
  }

  if (fgets(password, password_size, fp) == NULL) {
    ctc_log_error("get password failed");
    pclose (fp);
    return -1;
  }
...
static void ctc_inc_conn_count(int &dec_conn_count)
{
  ...
  dec_conn_count--;
  ctc_log_warning("[CTC_CONN_INC]:connection_count:%u, max_used_connections:%lu, max_connections:%lu, dec_conn_count:%d",
...
static void ctc_dec_conn_count(int &dec_conn_count)
{
  Connection_handler_manager::dec_connection_count();
  dec_conn_count++;
  ctc_log_warning("[CTC_CONN_DEC]:connection_count:%u, max_used_connections:%lu, max_connections:%lu, dec_conn_count:%d",
                  Connection_handler_manager::connection_count, Connection_handler_manager::max_used_connections, max_connections, dec_conn_count);
}
...
int ctc_mysql_conn(MYSQL *&con, const char *host, const char *user, const char *passwd) {
  con = mysql_init(NULL);
  if (con == nullptr) {
    ctc_log_error("ctc_mysql_conn mysql_init failed, error_code:%d,error:%s", mysql_errno(con), mysql_error(con));
    my_thread_end();
    if (!mysql_errno(con)) {
  ...
  */
  ...
  while (!mysql_real_connect(con, host, user, passwd, NULL, mysqld_port, mysqld_unix_port, CLIENT_MULTI_STATEMENTS | CLIENT_MULTI_RESULTS | CLIENT_REMEMBER_OPTIONS | CLIENT_INTERACTIVE) && retry_time > 0) {
    ...
    if (err == ER_CON_COUNT_ERROR) {
      ctc_dec_conn_count(dec_conn_count);
    }
    ...
    ctc_log_error("ctc_mysql_conn create internal connection failed, retry_time=%d, user=%s, err_code=%d, err_msg=%s.",
                  retry_time, user, err, mysql_error(con));
    retry_time--;
  }

  while (dec_conn_count != 0) {
    ctc_inc_conn_count(dec_conn_count);
  }
  my_thread_end();
  return con == nullptr;
}
...
int ctc_init_agent_client(MYSQL *&curr_conn) {
  SENSI_INFO char agent_password[CTC_PASSWORD_BUFFER_SIZE] = {0};
  string agent_user = "root";
  if (!ctc_get_agent_info(agent_password, CTC_PASSWORD_BUFFER_SIZE)) {
    agent_user = "RDS_agent";
  }
  const char *password = (agent_user == "root") ? NULL : agent_password;
  int ret = ctc_mysql_conn(curr_conn, my_localhost, agent_user.c_str(), password);
  memset(agent_password, 0, CTC_PASSWORD_BUFFER_SIZE);
  if (ret) {
    ctc_log_error("ctc_init_agent_client ctc_mysql_conn failed, err_code=%d, err_msg=%s.",
...
int ctc_mysql_query(MYSQL *mysql, const char *query) {
  ...
  */
  ...
  if (ctc_get_cluster_role() == (int32_t)dis_cluster_role::STANDBY) {
  ...
  if (ret != 0) {
    ctc_log_error("[CTC_MYSQL_QUERY]:ret:%d, err_code=%d, err_msg=%s, query_str:%s.", ret, mysql_errno(mysql), mysql_error(mysql), query);
  }
