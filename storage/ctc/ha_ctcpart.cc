...
// this module should use ctc_part_srv rather than knl_intf
  ...
  *used_parts = part_info->num_partitions_used();
  if (*used_parts > 0) {
    *part_ids = (uint32_t *)my_malloc(PSI_NOT_INSTRUMENTED, (*used_parts) * sizeof(uint32_t), MYF(MY_WME));
     if (*part_ids == NULL) {
      ctc_log_system("Failed to allocate memory for part_ids.");
      return false;
    }
    *subpart_ids = (uint32_t *)my_malloc(PSI_NOT_INSTRUMENTED, (*used_parts) * sizeof(uint32_t), MYF(MY_WME));
    if (*subpart_ids == NULL) {
      ctc_log_system("Failed to allocate memory for subpart_ids. Freeing previously allocated memory...");
      my_free(*part_ids);
      return false;
    }
    ...
    ctc_log_system("invalid used_parts :%u", *used_parts);
  }
  return true;
}

ha_ctcpart::ha_ctcpart(handlerton *hton, TABLE_SHARE *table_arg)
    : ha_ctc(hton, table_arg), Partition_helper(this),
...
/**
 ...
 @retval 0 if success
*/
int ha_ctcpart::open(const char *name, int mode, uint test_if_locked,
                     const dd::Table *table_def) {
  if (!(m_part_share = get_share<Ctcpart_share>())) return HA_ERR_OUT_OF_MEM;
  ...
  int ret = ha_ctc::open(name, mode, test_if_locked, table_def);
...
void ha_ctcpart::set_partition(uint part_id) {
...
void ha_ctcpart::get_auto_increment(ulonglong, ulonglong, ulonglong,
                                    ...
                                    ulonglong*) {
  uint64 inc_value;
  update_member_tch(m_tch, get_ctc_hton(), ha_thd());
  THD* thd = ha_thd();
  dml_flag_t flag;
  ...
  int ret = ctc_get_serial_value(&m_tch, &inc_value, flag);
  update_sess_ctx_by_tch(m_tch, get_ctc_hton(), ha_thd());
  if (ret != 0) {
...
void ha_ctcpart::print_error(int error, myf errflag) {
  if (print_partition_error(error)) {
    handler::print_error(error, errflag);
  }
}

void ha_ctcpart::part_autoinc_has_expl_non_null_value_update_row(uchar *new_data) {
...
/**
 ...
 @return error code.
*/
int ha_ctcpart::write_row_in_part(uint part_id, uchar *record) {
  ...
  int ret = ha_ctc::write_row(record);
...
void ha_ctcpart::start_bulk_insert(ha_rows rows) {
  assert(m_rec_buf_4_writing == nullptr);
  ha_ctc::start_bulk_insert(rows);
...
int ha_ctcpart::bulk_insert_low(dml_flag_t flag, uint *dup_offset) {
  record_info_t record_info = {m_rec_buf_data, (uint16_t)m_cantian_rec_len, nullptr, nullptr};
  return (ct_errno_t)ctc_bulk_write(&m_tch, &record_info, m_rec_buf_4_writing->records(),
...
*/
int ha_ctcpart::update_row_in_part(uint part_id, const uchar *old_row, uchar *new_row) {
  if (m_index_sorted) {
    cursor_set_bit(cursor_set, part_id);
    m_tch.cursor_addr = cursors[part_id];
  }
  int ret;
  ret = ha_ctc::update_row(old_row, new_row);
  reset_partition(part_id);
  return ret;
}
...
*/
int ha_ctcpart::write_row_in_new_part(uint new_part_id) {
...
/**
 ...
 @return error number or 0.
*/
int ha_ctcpart::delete_row_in_part(uint part_id, const uchar *record) {
  if (m_index_sorted) {
    cursor_set_bit(cursor_set, part_id);
    m_tch.cursor_addr = cursors[part_id];
  }
  int ret;
  ret = ha_ctc::delete_row(record);
  reset_partition(part_id);
  return ret;
}
...
/**
 ...
 @return error number or 0.
*/
int ha_ctcpart::rnd_init_in_part(uint part_id, bool scan) {
  set_partition(part_id);
  int ret = ha_ctc::rnd_init(scan);
  reset_partition(part_id);
  return ret;
}
...
/**
 ...
 @return error number or 0.
*/
int ha_ctcpart::rnd_next_in_part(uint part_id, uchar *buf) {
  set_partition(part_id);
  int ret = ha_ctc::rnd_next(buf);
  reset_partition(part_id);
  return ret;
}
...
/**
 ...
 @return error number or 0.
*/
int ha_ctcpart::rnd_end_in_part(uint part_id, bool scan) {
  UNUSED_PARAM(scan);
  if (m_tch.cursor_addr == INVALID_VALUE64) {
    return 0;
  }
  set_partition(part_id);
  int ret = ha_ctc::rnd_end();
  reset_partition(part_id);
  return ret;
}
...
*/
void ha_ctcpart::position_in_last_part(uchar *ref_arg, const uchar *record) {
  UNUSED_PARAM(record);
  ...
  if (cur_pos_in_buf == INVALID_MAX_UINT32) {
    ret = ctc_position(&m_tch, ref_arg, ref_length - PARTITION_BYTES_IN_POS);
  } else {
  ...
  if (ret != 0) {
     ctc_log_error("find position failed.");
     assert(0);
  }
  reset_partition(part_id);
}
...
*/
int ha_ctcpart::rnd_pos(uchar *buf, uchar *pos) {
  DBUG_TRACE;
  ha_statistic_increment(&System_status_var::ha_read_rnd_count);
  m_tch.thd_id = ha_thd()->thread_id();
  uint part_id = uint2korr(pos);
  set_partition(part_id);
  int ret = ha_ctc::rnd_pos(buf, pos + PARTITION_BYTES_IN_POS);
  reset_partition(part_id);
  return ret;
}
...
*/
int ha_ctcpart::index_init(uint index, bool sorted) {
  int ret;
  ...
  set_partition(part_id);
  ret = ha_ctc::index_init(index, sorted);
  reset_partition(part_id);
...
int ha_ctcpart::index_read(uchar *buf, const uchar *key, uint key_len,
                           ha_rkey_function find_flag) {
  return ha_ctc::index_read(buf, key, key_len, find_flag);
}
...
/**
 ...
 @return error number or 0.
*/
int ha_ctcpart::index_first_in_part(uint part_id, uchar *record) {
  if (part_id != m_last_part) {
    ha_ctc::reset_rec_buf();
  }
  set_partition(part_id);
  int ret = ha_ctc::index_first(record);
  reset_partition(part_id);
  if (m_index_sorted) {
    cursor_set_bit(cursor_set, part_id);
...
/**
 ...
 @return error number or 0.
*/
int ha_ctcpart::index_last_in_part(uint part_id, uchar *record) {
  if (part_id != m_last_part) {
    ha_ctc::reset_rec_buf();
  }
  set_partition(part_id);
  int ret = ha_ctc::index_last(record);
  reset_partition(part_id);
  if (m_index_sorted) {
    cursor_set_bit(cursor_set, part_id);
    cursors[part_id] = m_tch.cursor_addr;
...
/**
 ...
 @return error number or 0.
*/
int ha_ctcpart::index_prev_in_part(uint part_id, uchar *record) {
  if (m_index_sorted) {
    cursor_set_bit(cursor_set, part_id);
    m_tch.cursor_addr = cursors[part_id];
  }
  set_partition(part_id);
  int ret = ha_ctc::index_prev(record);
  reset_partition(part_id);
  return ret;
}
...
/**
 ...
 @return error number or 0.
*/
int ha_ctcpart::index_next_in_part(uint part_id, uchar *record) {
  if (m_index_sorted) {
    cursor_set_bit(cursor_set, part_id);
    m_tch.cursor_addr = cursors[part_id];
  }
  set_partition(part_id);
  int ret = ha_ctc::index_next(record);
  reset_partition(part_id);
  return ret;
}
...
/**
 ...
 @return error number or 0.
*/
int ha_ctcpart::index_next_same_in_part(uint part_id, uchar *record, const uchar *key, uint length) {
  if (m_index_sorted) {
    cursor_set_bit(cursor_set, part_id);
    m_tch.cursor_addr = cursors[part_id];
  }
  set_partition(part_id);
  int ret = ha_ctc::index_next_same(record, key, length);
  reset_partition(part_id);
  return ret;
}
...
/**
 ...
 @return error number or 0.
*/
int ha_ctcpart::index_read_map_in_part(uint part_id, uchar *record, const uchar *key,
                            key_part_map keypart_map, enum ha_rkey_function find_flag) {
  if (part_id != m_last_part) {
    ha_ctc::reset_rec_buf();
  }
  set_partition(part_id);
  int ret = ha_ctc::index_read_map(record, key, keypart_map, find_flag);
  reset_partition(part_id);
  if (m_index_sorted) {
    cursor_set_bit(cursor_set, part_id);
    cursors[part_id] = m_tch.cursor_addr;
...
/**
 ...
 @return error number or 0.
*/
int ha_ctcpart::index_read_last_map_in_part(uint part_id, uchar *record, const uchar *key,
                                key_part_map keypart_map) {
  if (part_id != m_last_part) {
    ha_ctc::reset_rec_buf();
  }
  set_partition(part_id);
  int ret = ha_ctc::index_read_last_map(record, key, keypart_map);
  reset_partition(part_id);
  if (m_index_sorted) {
    cursor_set_bit(cursor_set, part_id);
    cursors[part_id] = m_tch.cursor_addr;
...
/**
 ...
 @return error number or 0.
*/
int ha_ctcpart::index_read_idx_map_in_part(uint part_id, uchar *record, uint index,
                                const uchar *key, key_part_map keypart_map,
                                enum ha_rkey_function find_flag) {
  set_partition(part_id);
  int ret = ha_ctc::index_read_idx_map(record, index, key, keypart_map, find_flag);
  reset_partition(part_id);
  return ret;
}

bool ha_ctcpart::need_prune_partitions_by_engine(const key_range *start_key, const key_range *end_key) {
...
bool ha_ctcpart::equal_range_on_part_field(const key_range *start_key, const key_range *end_key) {
...
/**
 ...
 @return error number or 0.
*/
int ha_ctcpart::read_range_first_in_part(uint part_id, uchar *record,
  ...
  if (m_start_key.key != nullptr) {
    ...
    ret = ha_ctc::index_first(read_record);
  }
...
/**
 ...
 @return error number or 0.
*/
int ha_ctcpart::read_range_next_in_part(uint part_id, uchar *record) {
  uchar *read_record = record;
  if (read_record == nullptr) {
    read_record = table->record[0];
  }
  ...
  set_partition(part_id);
  int ret = ha_ctc::index_next(read_record);
  reset_partition(part_id);
  return ret;
}
...
*/
enum row_type ha_ctcpart::get_partition_row_type(const dd::Table *partition_table, uint part_id) {
...
double ha_ctcpart::scan_time() {
    ...
    if (m_part_share && m_part_share->cbo_stats != nullptr) {
        /*
        For partitioned tables, queries can specify partitions to scan.
        The cost of a full table scan should only include the specified partitions.
        */
        uint part_num = m_is_sub_partitioned ? 
                        table->part_info->num_parts * table->part_info->num_subparts : 
                        table->part_info->num_parts;
        for (uint i = m_part_info->get_first_used_partition(); i < part_num; 
            i = m_part_info->get_next_used_partition(i)) {
            scan_time += m_part_share->cbo_stats->ctc_cbo_stats_table[i].blocks;
        }
    }
    return scan_time;
}
...
void ha_ctcpart::info_low() {
    ...
    if (m_part_share && m_part_share->cbo_stats != nullptr) {
        uint total_part_num = m_is_sub_partitioned ? 
                              table->part_info->num_parts * table->part_info->num_subparts :
                              table->part_info->num_parts;
...
int ha_ctcpart::info(uint flag) {
  return ha_ctc::info(flag);
}

ha_rows ha_ctcpart::records_in_range(uint inx, key_range *min_key, key_range *max_key) {
...
for (uint part_id = m_part_info->get_first_used_partition(); part_id < part_num;
    ...
    density = calc_density_one_table(inx, &key, &m_part_share->cbo_stats->ctc_cbo_stats_table[part_id], *table);
    n_rows_num += m_part_share->cbo_stats->ctc_cbo_stats_table[part_id].estimate_rows * density;
  }
...
int ha_ctcpart::records(ha_rows *num_rows) /*!< out: number of rows */
{
  *num_rows = 0;
  int ret;
...
int ha_ctcpart::truncate_partition_low(dd::Table *dd_table) {
  ...
  ret = (ct_errno_t)ctc_truncate_partition(ctc_ddl_req_msg_mem, &ddl_ctrl);
  ctc_ddl_hook_cantian_error("ctc_truncate_partition_cantian_error", thd, &ddl_ctrl, &ret);
  m_tch = ddl_ctrl.tch;
  update_sess_ctx_by_tch(m_tch, ht, thd);
  return ctc_ddl_handle_fault("ctc_truncate_partition", thd, &ddl_ctrl, ret);
}

int ha_ctcpart::analyze(THD *thd, HA_CHECK_OPT *opt) {
  if (engine_ddl_passthru(thd)) {
  ...
  for (uint i = 0; i < used_parts; i++) {
    ...
    m_part_share->need_fetch_cbo = true;
    ret = ha_ctc::analyze(thd, opt);
    if (ret != 0) {
      ctc_log_error("analyze partition error!");
      my_free(subpart_ids);
      my_free(part_ids);
      return ret;
    }
...
int ha_ctcpart::optimize(THD *thd, HA_CHECK_OPT *opt) {
    int ret;
    m_tch.part_id = INVALID_PART_ID;
    ret = ha_ctc::analyze(thd, opt);
    if (ret != 0) {
    ...
    m_part_share->need_fetch_cbo = true;
    ret = ha_ctc::optimize(thd, opt);
    if (ret != 0) {
      ctc_log_error("optimize partition error!");
      return ret;
    }
...
int ha_ctcpart::initialize_cbo_stats() {
  ...
  *m_part_share->cbo_stats = {0, 0, 0, 0, 0, nullptr, 0, nullptr, nullptr, 0};
  ...
  if (ret != CT_SUCCESS) {
    ctc_log_error("m_part_share:ctc alloc str mysql mem failed, ret:%d", ret);
  }
  
  m_part_share->cbo_stats->msg_len = table->s->fields * sizeof(ctc_cbo_stats_column_t);
  m_part_share->cbo_stats->key_len = table->s->keys * sizeof(uint32_t) * MAX_KEY_COLUMNS;

  return CT_SUCCESS;
}

int ha_ctcpart::get_cbo_stats_4share()
{
  THD *thd = ha_thd();
  int ret = CT_SUCCESS;
  ...
  if (m_part_share->need_fetch_cbo || now - m_part_share->get_cbo_time > ctc_update_analyze_time) {
    if (m_tch.ctx_addr == INVALID_VALUE64) {
      ...
      update_member_tch(m_tch, get_ctc_hton(), thd);
      ret = ctc_open_table(&m_tch, table->s->table_name.str, user_name);
      update_sess_ctx_by_tch(m_tch, get_ctc_hton(), thd);
      if (ret != CT_SUCCESS) {
        return ret;
      }
    }
    ...
    for (uint32_t i = 0; i < fetch_times; i++) {
      update_member_tch(m_tch, get_ctc_hton(), thd);
      ret = ctc_get_cbo_stats(&m_tch, m_part_share->cbo_stats, &m_part_share->cbo_stats->ctc_cbo_stats_table[first_partid], first_partid, num_part_fetch);
      update_sess_ctx_by_tch(m_tch, get_ctc_hton(), thd);
      if (ret != CT_SUCCESS) {
        return ret;
      }
    ...
    if (ret == CT_SUCCESS && m_part_share->cbo_stats->is_updated) {
      ...
      ctc_index_stats_update(table, m_part_share->cbo_stats);
    }
...
void ha_ctcpart::free_cbo_stats() {
  if (m_part_share->cbo_stats == nullptr) {
      return;
  }
  uint32_t part_num = m_is_sub_partitioned ? table->part_info->num_parts * table->part_info->num_subparts :
  ...
  for (uint i = 0; i < part_num; i++) {
    free_columns_cbo_stats(m_part_share->cbo_stats->ctc_cbo_stats_table[i].columns, &is_str_first_addr, table);
  }
  ...
  my_free(m_part_share->cbo_stats->ctc_cbo_stats_table);
  m_part_share->cbo_stats->ctc_cbo_stats_table = nullptr;
  my_free(m_part_share->cbo_stats);
  m_part_share->cbo_stats = nullptr;
}

int ha_ctcpart::check(THD *, HA_CHECK_OPT *)
{
  return HA_ADMIN_OK;
}

int ha_ctcpart::repair(THD *thd, HA_CHECK_OPT *)
{
  if (engine_ddl_passthru(thd)) {
    return HA_ADMIN_OK;
  }
...
uint32 ha_ctcpart::calculate_key_hash_value(Field **field_array)
{
  return (Partition_helper::ph_calculate_key_hash_value(field_array));
}

bool ha_ctcpart::check_unsupported_indexdir(dd::Table *table_def) {
