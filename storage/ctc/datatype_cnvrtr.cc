...
static field_cnvrt_aux_t g_field_cnvrt_aux_array[] = {
  {MYSQL_TYPE_DECIMAL,     CANTIAN_COL_BITS_NULL, UNKNOW_DATA,   CTC_DDL_TYPE_DECIMAL    },
  {MYSQL_TYPE_TINY,        CANTIAN_COL_BITS_4,    NUMERIC_DATA,  CTC_DDL_TYPE_LONG       },
  {MYSQL_TYPE_SHORT,       CANTIAN_COL_BITS_4,    NUMERIC_DATA,  CTC_DDL_TYPE_LONG       },
  {MYSQL_TYPE_LONG,        CANTIAN_COL_BITS_4,    NUMERIC_DATA,  CTC_DDL_TYPE_LONG       },
  {MYSQL_TYPE_FLOAT,       CANTIAN_COL_BITS_8,    NUMERIC_DATA,  CTC_DDL_TYPE_DOUBLE     },
  {MYSQL_TYPE_DOUBLE,      CANTIAN_COL_BITS_8,    NUMERIC_DATA,  CTC_DDL_TYPE_DOUBLE     },
  {MYSQL_TYPE_NULL,        CANTIAN_COL_BITS_NULL, UNKNOW_DATA,   CTC_DDL_TYPE_NULL       },
  {MYSQL_TYPE_TIMESTAMP,   CANTIAN_COL_BITS_8,    DATETIME_DATA, CTC_DDL_TYPE_TIMESTAMP  },
  {MYSQL_TYPE_LONGLONG,    CANTIAN_COL_BITS_8,    NUMERIC_DATA,  CTC_DDL_TYPE_LONGLONG   },
  {MYSQL_TYPE_INT24,       CANTIAN_COL_BITS_4,    NUMERIC_DATA,  CTC_DDL_TYPE_LONG       },
  {MYSQL_TYPE_DATE,        CANTIAN_COL_BITS_8,    DATETIME_DATA, CTC_DDL_TYPE_DATE       },
  {MYSQL_TYPE_TIME,        CANTIAN_COL_BITS_8,    DATETIME_DATA, CTC_DDL_TYPE_TIME       },
  {MYSQL_TYPE_DATETIME,    CANTIAN_COL_BITS_8,    DATETIME_DATA, CTC_DDL_TYPE_DATETIME   },
  {MYSQL_TYPE_YEAR,        CANTIAN_COL_BITS_8,    DATETIME_DATA, CTC_DDL_TYPE_YEAR       },
  {MYSQL_TYPE_NEWDATE,     CANTIAN_COL_BITS_NULL, UNKNOW_DATA,   CTC_DDL_TYPE_NEWDATE    },
  {MYSQL_TYPE_VARCHAR,     CANTIAN_COL_BITS_VAR,  STRING_DATA,   CTC_DDL_TYPE_VARCHAR    },
  {MYSQL_TYPE_BIT,         CANTIAN_COL_BITS_8,  NUMERIC_DATA,   CTC_DDL_TYPE_LONGLONG   },
  {MYSQL_TYPE_TIMESTAMP2,  CANTIAN_COL_BITS_NULL, UNKNOW_DATA,   CTC_DDL_TYPE_TIMESTAMP2 },
  {MYSQL_TYPE_DATETIME2,   CANTIAN_COL_BITS_NULL, UNKNOW_DATA,   CTC_DDL_TYPE_DATETIME2  },
  {MYSQL_TYPE_TIME2,       CANTIAN_COL_BITS_NULL, UNKNOW_DATA,   CTC_DDL_TYPE_TIME2      },
  {MYSQL_TYPE_TYPED_ARRAY, CANTIAN_COL_BITS_NULL, UNKNOW_DATA,   CTC_DDL_TYPE_TYPED_ARRAY},
  // > MYSQL_TYPE_INVALID
  {MYSQL_TYPE_JSON,        CANTIAN_COL_BITS_VAR,  LOB_DATA,      CTC_DDL_TYPE_JSON       },
  {MYSQL_TYPE_NEWDECIMAL,  CANTIAN_COL_BITS_VAR,  NUMERIC_DATA,  CTC_DDL_TYPE_NEWDECIMAL },
  {MYSQL_TYPE_TINY_BLOB,   CANTIAN_COL_BITS_VAR,  LOB_DATA,      CTC_DDL_TYPE_TINY_BLOB  },
  {MYSQL_TYPE_MEDIUM_BLOB, CANTIAN_COL_BITS_VAR,  LOB_DATA,      CTC_DDL_TYPE_MEDIUM_BLOB},
  {MYSQL_TYPE_LONG_BLOB,   CANTIAN_COL_BITS_VAR,  LOB_DATA,      CTC_DDL_TYPE_LONG_BLOB  },
  {MYSQL_TYPE_BLOB,        CANTIAN_COL_BITS_VAR,  LOB_DATA,      CTC_DDL_TYPE_BLOB       },
  // The following two types not used for datatype convert, Unconfirmed scenarios in which this feature will be used.
  {MYSQL_TYPE_VAR_STRING,  CANTIAN_COL_BITS_NULL, UNKNOW_DATA,   CTC_DDL_TYPE_VAR_STRING },
  {MYSQL_TYPE_STRING,      CANTIAN_COL_BITS_VAR,  STRING_DATA,   CTC_DDL_TYPE_STRING     }
};
/**
...
const field_cnvrt_aux_t* get_auxiliary_for_field_convert(Field *field, enum_field_types mysql_field_type)
{
  ...
  if (field->is_flag_set(BLOB_FLAG)) {
    if (field->charset() == &my_charset_bin &&
      ...
      ret->ddl_field_type = CTC_DDL_TYPE_BLOB;
    } else {
      ret->ddl_field_type = CTC_DDL_TYPE_CLOB;
    }
  }
...
int decimal_mysql_to_double(const uint8_t *mysql_ptr, uchar *double_ptr, Field *mysql_field)
{
  ...
  ret = binary2my_decimal(E_DEC_FATAL_ERROR, mysql_ptr, &d, prec, scale);
  if (ret != E_DEC_OK) {
    ctc_log_error("[decimal2double]: Decimal data type convert binary to my_decimal failed!");
    return ret;
  }
  ret = decimal2double(&d, (double *)double_ptr);
  if (ret != E_DEC_OK) {
    ctc_log_error("[decimal2double]: Decimal data type convert my_decimal to double failed!");
  }
  return ret;
}
...
int convert_blob_to_cantian(uchar *cantian_ptr, uint32 &cantian_offset,
  ...
  // for inline
  if (remain_size <= CTC_LOB_MAX_INLINE_SIZE) {
  ...
  while (remain_size > 0) {
    ...
    int ret = ctc_knl_write_lob(&tch, (char *)lob_locator, locator_size, column_id,
                                (void *)piece_lob.str, (uint32_t)piece_lob.len, force_outline);
    if (ret != CT_SUCCESS) {
      ctc_log_error("[mysql2cantian]ctc_knl_write_lob Failed:%d", ret);
      my_free(lob_locator);
      lob_locator = nullptr;
      field_len = 0;
...
void convert_json_to_mysql(Field *mysql_field)
{
  ...
  if (mysql_field->type() == MYSQL_TYPE_JSON) {
    ...
    if (json == nullptr || (bitmap_is_set(json->table->read_set, mysql_field->field_index()) && json->val_json(&wr))) {
      ctc_log_error("[convert_json_to_mysql] get json value failed");
      assert(0);
    }
  }
  return;
...
/**
  @brief
  The blob data stored in the CTC storage engine can be in-line or out-line scenarios.
  However, you can query the blob data by calling the ctc_knl_read_lob interface in
...
*/
void convert_blob_to_mysql(uchar *cantian_ptr, Field *mysql_field, ctc_handler_t &tch, uint8_t *mysql_ptr)
{
  ...
  if (blob_len == 0) {
    ctc_log_note("[cantian2mysql]data type %d for 0 length", mysql_field->type());
  ...
  if (blob_buf == nullptr) {
    ctc_log_error("[cantian2mysql]Apply for blob buf:%u Failed", blob_len);
  ...
  while (remain_size > 0) {
    ...
    int ret = ctc_knl_read_lob(&tch, (char *)locator, offset, blob_buf + offset, piece_len, &read_size);
    if (ret != CT_SUCCESS) {
      ctc_log_error("[cantian2mysql]ctc_knl_read_lob Failed : %d", ret);
      read_flag = false;
      break;
    }
    if (read_size <= 0) {
      ctc_log_note("[cantian2mysql]ctc_knl_read_lob read lob length is %d,stop read!", read_size);
      read_flag = false;
      break;
    }
...
longlong bit_cnvt_mysql_cantian(const uchar *ptr, Field *mysql_field)
{
  ...
  if (field == nullptr) {
    ctc_log_error("bit_cnvt_mysql_cantian failed!");
    assert(0);
  }
...
void bit_cnvt_cantian_mysql(const uchar *cantian_ptr, uchar *mysql_ptr, Field *mysql_field)
{
  ...
  Field_bit *field = dynamic_cast<Field_bit *>(mysql_field);
  if (field == nullptr) {
    ctc_log_error("bit_cnvt_cantian_mysql failed!");
    assert(0);
    return;
  }
  uint bytes_in_rec = field->bytes_in_rec;
                                  ...
                                  const uchar *mysql_ptr, Field *mysql_field)
{
  switch (mysql_info->mysql_field_type) {
    ...
    case MYSQL_TYPE_TIMESTAMP:
      ...
      break;
    default:
      ctc_log_error("[mysql2cantian datetime]unsupport datatype %d", mysql_info->mysql_field_type);
      assert(false);
  }
}
                                ...
                                const uchar *mysql_ptr, Field *mysql_field)
{
  switch (mysql_info->mysql_field_type) {
    case MYSQL_TYPE_TIME:
      cnvrt_time_decimal(mysql_ptr, mysql_field->decimals(), cantian_ptr, DATETIME_MAX_DECIMALS, CTC_BYTE_8);
      break;

    case MYSQL_TYPE_DATETIME:
      cnvrt_datetime_decimal(mysql_ptr, mysql_field->decimals(), cantian_ptr, DATETIME_MAX_DECIMALS, CTC_BYTE_8);
      break;

    case MYSQL_TYPE_DATE:
      memset(cantian_ptr, 0, CTC_BYTE_8);
...
*/
bool ctc_datetime_with_no_zero_in_date_to_timeval(const MYSQL_TIME *ltime,
                                                         const Time_zone &tz,
                                                         struct timeval *tm,
                                                         int *warnings)
{
...
/**
  ...
  @param        dec  Precision.
*/
static void ctc_my_timestamp_to_binary(const struct timeval *tm, uchar *ptr, uint dec)
{
...
*/
static void convert_datetime_to_mysql(ctc_handler_t &tch, const field_cnvrt_aux_t* mysql_info,
  uint8_t *mysql_ptr, uchar *cantian_ptr, Field *mysql_field)
{
  ...
  switch (mysql_info->mysql_field_type) {
    ...
    case MYSQL_TYPE_TIMESTAMP: {
      ...
      ctc_datetime_with_no_zero_in_date_to_timeval(&ltime, *my_tz_OFFSET0, &tm, &warnings);
      // Assume that since the value was properly stored, there're no warnings
      assert(!warnings);
      ctc_my_timestamp_to_binary(&tm, mysql_ptr, dec);
      break;
    }
    default:
      ctc_log_error("[cantian2mysql datetime]unsupport datatype %d", mysql_info->mysql_field_type);
      assert(0);
      break;
  }
}
  ...
  switch (mysql_info->mysql_field_type) {
    case MYSQL_TYPE_VARCHAR:
      ...
      if (data_offset == 1) {
        ...
        ctc_log_error("[mysql2cantian]get varchar data length error: %u", data_offset);
      }
      break;
    ...
    case MYSQL_TYPE_NEWDECIMAL: {
      ...
      Field_new_decimal *f = dynamic_cast<Field_new_decimal *>(mysql_field);
      if (f == nullptr) {
        ctc_log_error("[mysql2cantian calculate length]unknow data type: %d", mysql_info->mysql_field_type);
        data_offset = 0;
        break;
      }
                                     ...
                                     uint8_t *mysql_ptr, uint32 field_len, Field *field)
{
  ...
  switch (mysql_info->mysql_field_type) {
    ...
    default:
      ctc_log_debug("[cantian2mysql]mysql field type: %d do not need data head", mysql_info->mysql_field_type);
      break;
  }
  return mysql_offset;
}
                                                 ...
                                                 Field *field, record_buf_info_t *record_buf)
{
  switch (mysql_info->cantian_map_type) {
    ...
    case CANTIAN_COL_BITS_VAR:
      ...
      break;
    default:
      ctc_log_error("[mysql2cantian]unknow col bits: %d", mysql_info->cantian_map_type);
      assert(0);
      break;
  }
}
...
static int convert_data_from_mysql_to_cantian(const field_cnvrt_aux_t* mysql_info, Field *field,
                                              ...
                                              field_offset_and_len_t *field_offset_info,
                                              uint32_t &field_len, ctc_handler_t &tch, uint column_id)
{
  int res = 0;
...
int mysql_record_to_cantian_record(const TABLE &table, record_buf_info_t *record_buf, ctc_handler_t &tch,
                               ...
                               ctc_handler_t &tch, bool is_index_only)
{
  ...
  switch (mysql_info->sql_data_type) {
    ...
    /* fall through */
    default:
      ctc_log_error("[cantian2mysql]unsupport datatype %d", mysql_info->sql_data_type);
      assert(0);
      break;
  }
}

static inline bool ctc_field_type_is_lob(bool is_index_only, const field_cnvrt_aux_t* cnvrt_aux, Field *field, ctc_handler_t &tch)
{
...
static bool ctc_update_offset_single(uint col_id, Field *field, uint8 cantian_col_type, const field_cnvrt_aux_t* cnvrt_aux, record_info_t *record_info,
                                     record_buf_info_t *record_buf, field_offset_and_len_t *size, bool is_index_only, ctc_handler_t &tch)
{
  switch (cantian_col_type) {
    ...
    default:
      ctc_log_error("[cantian2mysql]unknow col bits: %u", cantian_col_type);
      assert(0);
      break;
  }
  return false;
                                    ...
                                    ctc_handler_t &tch, record_info_t *record_info)
{
  ...
  ctc_log_debug("size %u, column cnt %u, is_changed:%hu, is_deleted:%hu, "
  ...
  if (row_head->is_csf) {
    ctc_log_error("csf format is not supported yet");
    assert(0);
  }

  if (IS_SPRS_ROW(row_head)) {
    ctc_log_error("sparse row not supported yet");
    assert(0);
  }
  ...
  for (Field **field = table->field; *field; field++) {
    ...
    switch (mysql_field->type()) {
      ...
      case MYSQL_TYPE_MEDIUM_BLOB:
        ...
        break;
      default:
        ctc_log_error("unsupported column type %d", mysql_field->type());
        return -1;
    }
