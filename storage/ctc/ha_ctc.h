...
#define CTC_MAX_KEY_PARTS 16         // CT_MAX_INDEX_COLUMNS
#define CTC_MAX_KEY_LENGTH 4095      // CT_MAX_KEY_SIZE
#define CTC_MAX_KEY_NUM 32           // CT_MAX_TABLE_INDEXES
...
static const uint CTC_START_TIMEOUT = 120; // seconds
extern const char *ctc_hton_name;
...
/** @brief
  Ctc_share is a class that will be shared among all open handlers.
*/
class Ctc_share : public Handler_share {
 public:
...
class ha_ctc : public handler {
public:
  ha_ctc(handlerton *hton, TABLE_SHARE *table);
  ~ha_ctc();
...
*/
  ...
  */
  ...
  void set_ctc_range_key(ctc_key *ctc_key, key_range *mysql_range_key, bool is_min_key);
...
*/
  ...
  */
  ...
  /** Check if Ctc supports a particular alter table in-place
  ...
  ctc_handler_t *get_m_tch() { return &m_tch; }
 ...
 protected:
  ...
  /* ctc handler that can be used to determine who sends the request */
  ctc_handler_t m_tch;

  /* cantian record buffer */
  uchar *m_ctc_buf = nullptr;
  ...
  /** Pointer to Ctc_share on the TABLE_SHARE. */
  Ctc_share *m_share;

  ctc_select_mode_t get_select_mode();

private:
  int ctc_alloc_ctc_buf_4_read();
...
int ha_ctc_get_inst_id();
void ha_ctc_set_inst_id(uint32_t inst_id);
...
handlerton *get_ctc_hton();
void update_sess_ctx_by_tch(ctc_handler_t &tch, handlerton *hton, THD *thd);
void update_member_tch(ctc_handler_t &tch, handlerton *hton, THD *thd, bool alloc_msg_buf = true);
int get_tch_in_handler_data(handlerton *hton, THD *thd, ctc_handler_t &tch, bool alloc_msg_buf = true);
