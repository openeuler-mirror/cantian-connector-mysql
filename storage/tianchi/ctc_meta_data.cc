...
static void ctc_init_thd(THD **thd, uint64_t thd_key) {
  ...
  if (g_tse_mdl_thd_map.find(thd_key) != g_tse_mdl_thd_map.end()) {
    ...
    THD* new_thd = new THD(next_thread_id());;
    my_thread_init();
#if 0
    ...
    new_thd->set_query(const_cast<char *>("tse_mdl_thd_notify"), 18);
...
typename std::enable_if<CHECK_HAS_MEMBER_FUNC(T, invalidates), bool>::type
  ...
  if(!thd->mdl_context.is_lock_owner(
  ...
  if (type == T::OBJ_RT_PROCEDURE) {
    sptype = SP_TYPE_PROCEDURE;
    mdl_key.mdl_key_init(MDL_key::PROCEDURE, const_cast<char*>(schema_name), const_cast<char*>(routine_name));
  } else {
    mdl_key.mdl_key_init(MDL_key::FUNCTION, const_cast<char*>(schema_name), const_cast<char*>(routine_name));
    sptype = SP_TYPE_FUNCTION;
  }

  if(!thd->mdl_context.is_lock_owner(&mdl_key, MDL_EXCLUSIVE)) {
    MDL_request mdl_request;
  ...
  LEX_CSTRING lex_routine_name = { routine_name, strlen(routine_name) };
  LEX_CSTRING lex_schema_name = { schema_name , strlen(schema_name ) };
  sp_name *name = new (thd->mem_root) sp_name(&lex_schema_name, &lex_routine_name, true);
  sp_cache_invalidate();
  sp_head *sp;
  sp_cache **spc = (sptype == SP_TYPE_FUNCTION ? &thd->sp_func_cache
                                                     : &thd->sp_proc_cache);
...
typename std::enable_if<CHECK_HAS_MEMBER_FUNC(T, invalidates), bool>::type
  ...
  // mariadb always removes all 
  tdc_remove_table(thd, /*TDC_RT_REMOVE_NOT_OWN,*/ schema_name, table_name);
...
static typename std::enable_if<CHECK_HAS_MEMBER_FUNC(T, invalidates), int>::type
  ...
  if (broadcast_req->is_dcl == true) {
    int not_used;
    error = reload_acl_and_cache(thd, REFRESH_GRANT, NULL, &not_used);
...
static void tse_init_mdl_request(tse_lock_table_info *lock_info, MDL_request *mdl_request) {
  MDL_key mdl_key;
  std::string schema_name (lock_info->db_name);
  std::string name (lock_info->table_name);
  MDL_key::enum_mdl_namespace tse_mdl_namespace = (MDL_key::enum_mdl_namespace)lock_info->mdl_namespace;
  mdl_key.mdl_key_init(tse_mdl_namespace, schema_name.c_str(), name.c_str());
  MDL_REQUEST_INIT_BY_KEY(mdl_request, &mdl_key, MDL_EXCLUSIVE, MDL_EXPLICIT);
...
int tse_mdl_lock_thd(tianchi_handler_t *tch, tse_lock_table_info *lock_info, int *err_code) {
  ...
  if (thd->mdl_context.acquire_lock(&tse_mdl_request, thd->variables.lock_wait_timeout)) {
...
static void ctc_get_set_var_item(THD* new_thd, sys_var* sysvar, Item** res MY_ATTRIBUTE((unused)), string& var_value,
  ...
  switch (sysvar->show_type()) {
    ...
    //case SHOW_HA_ROWS:
      if (var_value.c_str()[0] != '-') {
        *res = new (new_thd->mem_root)
          Item_uint(new_thd, var_value.c_str(), (uint)var_value.length());
      } else {
        *res = new (new_thd->mem_root)
          Item_int(new_thd, var_value.c_str(), (uint)var_value.length());
	  }
      break;
    case SHOW_BOOL:
    //case SHOW_MY_BOOL:
      if(var_value == "1" || var_value == "0") {
        ...
        *res = new (new_thd->mem_root)
            Item_string(new_thd, var_value.c_str(), var_value.length(),
    ...
    case SHOW_CHAR_PTR:
      if (is_null_value)
        *res = new (new_thd->mem_root) Item_null(new_thd);
      else
        *res = new (new_thd->mem_root)
            Item_string(new_thd, var_value.c_str(), var_value.length(),
                        &my_charset_utf8mb4_bin);
    ...
    case SHOW_DOUBLE:
      *res = new (new_thd->mem_root)
          Item_float(new_thd, var_value.c_str(), (uint)var_value.length());
      break;
    default:
...
static void ctc_set_var_type(uint32_t option, set_var *var) {
  if ((option & TSE_SET_VARIABLE_PERSIST) > 0) {
	...
	DBUG_ASSERT(false);// not supported on mariadb
  }
...
static void ctc_init_thd_priv(THD** thd, Sctx_ptr<Security_context> *ctx) {
  ...
  /*const std::vector<std::string> priv_list = {
      ...
      "AUDIT_ADMIN"}; the prv bits in mariadb are different from mysql */
  const privilege_t priv_list = (SET_USER_ACL | CREATE_USER_ACL | SUPER_ACL);
  const privilege_t static_priv_list = (SUPER_ACL | FILE_ACL);
  // 
  Security_context *sctx = new Security_context();
  memcpy(sctx->priv_user, "bootstrap", sizeof("bootstrap"));
  memcpy(sctx->priv_host, "localhost", sizeof("localhost"));
  sctx->user = sctx->priv_user;
  sctx->host = sctx->priv_host;
  sctx->master_access = priv_list;
  sctx->db_access = static_priv_list;

  (*ctx) = Sctx_ptr<Security_context>(sctx);
  /* attach this auth id to current security_context */
  new_thd->set_security_context(ctx->get());
  new_thd->real_id = pthread_self();
  (*thd) = new_thd;
}
...
#ifdef FEATURE_X_FOR_MYSQL_26
  var = new (new_thd->mem_root) set_var(new_thd, OPT_GLOBAL, sysvar, &base_name, res);
