...
*/
inline uint64_t tse_get_conn_key(uint32_t inst_id, uint32_t thd_id, bool use_proxy) {
...
inline uint32_t tse_get_inst_id_from_conn_key(uint64_t conn_key) {
  return (conn_key & (AGENT_CONN_KEY_MASK - 1)) >> 32;
}

inline uint16_t tse_get_cantian_id_from_conn_key(uint64_t conn_key) {
