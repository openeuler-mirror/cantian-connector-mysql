...
static void tse_index_make_up_key_length(int *key, uint8_t **origin_key, uint32_t *origin_key_len, uint32_t length) {
  int tmp_key = 0;
...
int tse_fill_index_key_info(TABLE *table, const uchar *key, uint key_len, const key_range *end_range, 
  ...
  do {
    ...
    tse_convert_mysql_key_to_cantian(key_part, key, key_len, &data_field_len,
...
int tse_convert_key_from_mysql_to_cantian(Field *field, uint8_t **mysql_ptr, dec4_t *cantian_ptr, uint32_t *len)
{
  int ret = CT_SUCCESS;
  ...
  if (mysql_info->mysql_field_type == MYSQL_TYPE_TINY || mysql_info->mysql_field_type == MYSQL_TYPE_SHORT ||
    ...
    tse_index_make_up_key_length(reinterpret_cast<int *>(cantian_ptr), mysql_ptr, len, sizeof(int));
  ...
  if (mysql_info->sql_data_type == DATETIME_DATA) {
    ...
    if (ret != CT_SUCCESS) {
      tse_log_error("tse convert index datatime to cantian failed.");
      return ret;
    }
  ...
  if (field->type() == MYSQL_TYPE_DECIMAL || field->type() == MYSQL_TYPE_NEWDECIMAL) {
    ...
    if (ret != CT_SUCCESS) {
      tse_log_error("tse convert index decimal to cantian failed.");
      return ret;
    }
...
int tse_convert_index_datatype(TABLE *table, index_key_info_t *index_key_info, bool has_right_key, dec4_t *data) {
  ...
  for (int i = 0; i < index_key_info->key_num; ++i) {
    ...
    if (index_key_info->key_info[i].left_key_len != 0) {
      ret = tse_convert_key_from_mysql_to_cantian(field, &index_key_info->key_info[i].left_key, data++, &index_key_info->key_info[i].left_key_len);
      if (ret != CT_SUCCESS) {
        tse_log_error("tse_convert_key_from_mysql_to_cantian: convert mysql index search left key failed, ret(%d).", ret);
        return ret;
      }
    }

    if (has_right_key && index_key_info->key_info[i].right_key_len != 0) {
      ret = tse_convert_key_from_mysql_to_cantian(field, &index_key_info->key_info[i].right_key, data++, &index_key_info->key_info[i].right_key_len);
      if (ret != CT_SUCCESS) {
        tse_log_error("tse_convert_key_from_mysql_to_cantian: convert mysql index search right key failed, ret(%d).", ret);
        return ret;
      }
    }
  }
